...
RUN PLATFORM=$(dpkg --print-architecture) && \
    ...
    # Google currently does not provide an arm64 build for Google Chrome
    # And the `chromium` package requires `snapd` which does not work in docker
    ...
    python3-num2words python3-ofxparse python3-openid python3-openpyxl python3-openssl python3-passlib \
    python3-pdfminer python3-phonenumbers python3-pil python3-polib python3-psutil python3-psycogreen python3-psycopg2 \
    python3-pydot python3-pyldap python3-pyparsing python3-pypdf2 python3-qrcode python3-renderpm python3-reportlab \
    ...
    python3-xlsxwriter python3-xlwt python3-xmlsec python3-zeep \
